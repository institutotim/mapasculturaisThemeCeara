...
module.controller('RegistrationConfigurationsController', ['$scope', '$rootScope', '$timeout', '$interval', 'RegistrationConfigurationService', 'EditBox', '$http', function ($scope, $rootScope, $timeout, $interval, RegistrationConfigurationService, EditBox, $http) {
    ...
    module.controller('ProjectController', ['$scope', '$rootScope', '$timeout', 'RegistrationService', 'EditBox', 'RelatedAgentsService', '$http', 'UrlService', function ($scope, $rootScope, $timeout, RegistrationService, EditBox, RelatedAgentsService, $http, UrlService) {
            ...
            $scope.showRegistration = function(registration){
                var status = !$scope.data.registrationStatus && $scope.data.registrationStatus !== 0 || $scope.data.registrationStatus === registration.status;
            ...
            // Retorna quantidade de inscrições por status no edital
            var getFilteredRegistrationsCount = function(status) {

              var count = 0;
              var registrations = $scope.data.entity.registrations;

              for (var key in registrations) {
                if (registrations[key].status === status) {
                  count++;
                }
              }
              return count;
            ...
            // Retorna quantidade de inscrições por status pendente no edital
            $scope.getFilteredRegistrationsPendingCount = function() {
              return getFilteredRegistrationsCount(STATUS.PENDING);
            }

            // Retorna quantidade de inscrições por status rascunho no edital
            $scope.getFilteredRegistrationsDraftCount = function() {
              return getFilteredRegistrationsCount(STATUS.DRAFT);
            }

            // Retorna quantidade de inscrições por status suplente no edital
            $scope.getFilteredRegistrationsWaitListCount = function() {
              return getFilteredRegistrationsCount(STATUS.WAITLIST);
            }

            // Retorna quantidade de inscrições por status aprovado no edital
            $scope.getFilteredRegistrationsAprovedCount = function() {
              return getFilteredRegistrationsCount(STATUS.APPROVED);
            }

            // Retorna quantidade de inscrições por status não selecionada no edital
            $scope.getFilteredRegistrationsNotaAprovedCount = function() {
              return getFilteredRegistrationsCount(STATUS.NOTAPPROVED);
            }

            // Retorna quantidade de inscrições por status inválida no edital
            $scope.getFilteredRegistrationsInvalidCount = function() {
              return getFilteredRegistrationsCount(STATUS.INVALID);
            }

            // Retorna se o filtro é igual ao status da lista para mostrar os count
            $scope.showStatusRegistration = function(status) {
                return $scope.data.registrationStatus === status;
            };
