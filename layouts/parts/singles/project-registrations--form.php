<?php if ($entity->isRegistrationOpen() && !$this->isEditable() && $entity->useRegistrations): ?>
    <?php if ($app->auth->isUserAuthenticated()): ?>
        ...
        <form id="project-registration" class="registration-form clearfix">
            ...
            <div>
                <div id="select-registration-owner-button" class="input-text" ng-click="editbox.open('editbox-select-registration-owner', $event)">{{data.registration.owner ? data.registration.owner.name : 'Agente responsável pelo Projeto'}}</div>
                <edit-box id="editbox-select-registration-owner" position="bottom" title="Selecione o agente responsável pelo Projeto." cancel-label="Cancelar" close-on-cancel='true' spinner-condition="data.registrationSpinner">
